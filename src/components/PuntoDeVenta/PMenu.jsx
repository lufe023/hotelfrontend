...
const PMenu = ({ busqueda, setBusqueda, favoritos, setProductos, carrito, setCarrito}) => {
  ...
  useEffect(() => {
    if (busqueda.length > 2 && inputMethod === 'manual') {
        ...
        debounceTimeout = setTimeout(() => {
            if (busqueda.trim() !== "") {  // Validar que la búsqueda no esté vacía
...
useEffect(() => {
  ...
  const checkDevice = () => {
    ...
    setIsMobile(
      /iphone|ipod|ipad|android/.test(userAgent) || window.innerWidth <= 800
    );
  };
    ...
    <div>
      <nav className="navbar navbar-main px-0 bg-white shadow-sm"
        ...
        <div className="container-fluid">
          <nav aria-label="breadcrumb" className="d-none d-md-block" style={{ width: 200 }}>
            <FechaHora />
            
          </nav>
          <div className="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4 align-items-left"
            id="navbar"
            style={{ minHeight: 50, display: "flex", justifyContent: "space-between", width: "80%" }}>
            <div className="ms-md-3 col-8" style={{ width: "100%", position: 'relative' }}>
              <input
                ref={inputRef}
                type="text"
                className="form-control w-100"
                placeholder="Buscar por nombre o código de barras"
                style={{ borderColor: inputMethod === "scanner" ? "green" : "" }}
                value={busqueda}
                onKeyDown={(e) => {
                  if (e.key === "Delete") {
                    handleClearInput();
